 ...
 * temporary file is immediately unlinked. Returns a handle 
 * to the temporary file.  */

temp_file_handle write_temp_file (char* buffer, size_t length)
{
	/* Create the filename and file. The XXXXXX will be replaced with
	 * characters that the filename unique.  */
...
/* Read the contents of a temporary file TEMP_FILE created with 
...
char* read_temp_file (temp_file_handle temp_file, size_t* length)
{
	...
	lseek (fd, 0, SEEK_SET);
	/* Read the size of data in the temporary file. */
...
/* tmpfile function create and open a temporary file ,
 * and return a file pointer to it. The temporary file is already unlinked.
 * Don't use other temp file function !  */
