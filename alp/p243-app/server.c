...
/* HTTP response, header, and body, indicating that we don't understand
 * the request. */
...
/* HTTP response, header, and body template, indicating that the request
...
/* HTTP response, header, and body template, indicating that the method
 * was not understand. */
...
"    <p>The method %s is not Implemented on this server.</p>\n"
"  </body>\n"
"</html>\n";
...
static void handle_get(int connection_fd, const char *page) {
	...
	/* Make sure the requested page begins with a slash and does not contain
	 * any additional slashes -- we don't support any subdirectories. */
	if (*page == '/' && strchr(page + 1, '/') == NULL) {
		char module_file_name[64];

		/* Append ".so" to the page name. */
	...
	if (module == NULL) {
		...
		/* Invoke the module, which will generate HTML output and sent it
...
void server_run(struct in_addr local_address, uint16_t port)
{
	 ...
	 * which we want to listen for connection. */
	memset(&socket_address, 0, sizeof (socket_address));
	socket_address.sin_family = AF_INET;
	socket_address.sin_port = port;
	socket_address.sin_addr = local_address;
	/* Bind the socket to that address. */
	rval = bind(server_socket, (const struct sockaddr *)&socket_address,
	...
	while (1) {
		...
		if (verbose) {
			...
			assert(rval == 0);
			/* Print a message. */
			printf("connections accepted from %s\n",
				   inet_ntoa(socket_address.sin_addr));
		}
		...
		if (child_pid == 0) {
			...
			close(STDIN_FILENO);
			close(STDOUT_FILENO);
			/* Also this process shouldn't do anything with the
			 * listening socket. */
			close(server_socket);
			 ...
			 * descriptor. Close it. Then continue with the loop and accept
			 * another connection. */
			close(connection);
		} else {
			/* Call to fork failed. */
