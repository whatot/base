...
static int get_uid_gid(pid_t pid, uid_t *uid, gid_t *gid)
{
	...
	if (rval != 0) {
		/* Couldn't dind it; perhaps this process no longer exists.*/
		return 1;
	}
...
/* Return the name of the program running in process PID, or NULL on error.
 * The return value is a newly allocated buffer which the caller must
...
static char *get_program_name(pid_t pid)
{
	...
	if (fd == -1) {
		/* Couldn't open the stat file for this process.
	...
	/* The program name is the second element of the file contents and
	 * is surrounded by parentheses. Find the positions of the parentheses
...
/* Return the resident set size (RSS), in kb, of process PID.
 * Return -1 on failure. */
static int get_rss(pid_t pid)
{
	...
	/* Generate the name of the process's "statm" entry in its /proc dir. */
...
static char *format_process_info(pid_t pid)
{
	...
	/* Compute the length of the string we'll need to hold the result,
...
void module_generate(int fd)
{
	...
	/* Set up an iovec array. We'll fill this with buffers that'll be part
	 * of output, growing it dynamically as necessary. */
