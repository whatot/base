...
struct server_module *module_open(const char *module_name)
{
	...
	/* Construct the full path of the module shared library
	 * we'll try to load. */
	...
	free(module_path);
	if (handle == NULL) {
		/* Failed; either this path doesn't exist or it isn't a shared lib. */
		return NULL;
	}

	/* Resolve the module_generate symbol from the shared library. */
	/* module_generate = (void (*)(int)) dlsym(handle, "module_generate");
	 * above function, but didn't fill -pedantic */
	void *temp = dlsym(handle, "module_generate");
	memcpy(&module_generate, &temp, sizeof(module_generate));
